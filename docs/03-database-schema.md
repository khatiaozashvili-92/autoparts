...
CREATE TABLE user_roles (
  ...
  partner_id uuid,                       -- PARTNER_* როლებისთვის სავალდებულო
  granted_at timestamptz NOT NULL DEFAULT now(),
  granted_by uuid REFERENCES users(id),
  PRIMARY KEY (user_id, role, COALESCE(partner_id, '00000000-0000-0000-0000-000000000000'::uuid))
);
...
CREATE TABLE master_parts (
  ...
  created_at  timestamptz NOT NULL DEFAULT now(),
  -- §94: duplicate master products აკრძალულია
  UNIQUE (category_id, normalized_name, COALESCE(position,''), COALESCE(axle,''))
);
...
CREATE TABLE offers (
  ...
  UNIQUE (partner_id, product_id, location_id)
);
